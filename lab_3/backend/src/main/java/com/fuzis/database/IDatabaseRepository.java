...
public interface IDatabaseRepository extends Serializable {
    ...
    default <T> T get(Integer id, Class<T> cls) {
        ...
        // Метод find автоматически использует L2 кэш благодаря @Cacheable
    ...
    default <T> List<T> getAll(Class<T> cls){
        ...
        query.setHint("org.hibernate.cacheable", true); // Включаем кэширование запроса
    ...
    default <T> List<T> getPage(int page, int count, Class<T> cls) {
        ...
        TypedQuery<T> query = this.getEntityManager().createQuery(
